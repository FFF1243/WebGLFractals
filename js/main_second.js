...
var vertixesCounts = 0;
var depth = 3;
var perturb = 5;
// funkcja główna 
function runWebGLSierp() {
    getRotation();
...
function sierpinski(length, vertices, faces, depth, left, right) {

    var middleSize = length / 3;
    for (var i = 0; i < 3; i++)
    {
        for (var j = 0; j < 3; j++)
        {
            if (i == 1 && j == 1)
            {
                vertices.push(j * middleSize + left);
                vertices.push(right - i * middleSize);
                vertices.push(0); //z
                ...
                var firstIndex = vertices.length / 6 - 1;

                vertices.push(j * middleSize + left);
                vertices.push(right - (i + 1) * middleSize);
                vertices.push(0); //z
                ...
                vertices.push((j + 1) * middleSize + left);
                vertices.push(right - i * middleSize);
                vertices.push(0); //z
                ...
                faces.push(firstIndex);
                faces.push(firstIndex + 1);
                faces.push(firstIndex + 2);

                faces.push(firstIndex + 1);
                faces.push(firstIndex + 2);
                faces.push(firstIndex + 3);
            ...
            else
            {
                ...
                if (depth == 1)
                {
                    ...
                    vertices.push((j * middleSize + left) + (middleSize * ((Math.random() - 0.5)/perturb)));
                    vertices.push((right - i * middleSize) + (middleSize * ((Math.random() - 0.5)/perturb)));
                    vertices.push(0); //z
                    ...
                    var firstIndex1 = vertices.length / 6 - 1;

                    vertices.push((j * middleSize + left) + (middleSize * ((Math.random() - 0.5)/perturb)));
                    vertices.push((right - (i + 1) * middleSize) + (middleSize * ((Math.random() - 0.5)/perturb)));
                    vertices.push(0); //z
                    ...
                    vertices.push(((j + 1) * middleSize + left) + (middleSize * ((Math.random() - 0.5)/perturb)));
                    vertices.push((right - i * middleSize) + (middleSize * ((Math.random() - 0.5)/perturb)));
                    vertices.push(0); //z
                ...
                {
                    sierpinski(middleSize, vertices, faces, depth - 1, j * middleSize + left, right - i * middleSize);
                }
            }
        }
    }
...
function gl_initBuffers () {    
    ...
    perturb = parseInt($("#pertubation").val());
     sierpinski(4, triangleVertices, triangleFaces, depth, -2, 2);
     vertixesCounts = triangleFaces.length;
...
function gl_draw() {
    ...
    var animate = function (time) {
       ...
       gl_ctx.bindBuffer(gl_ctx.ARRAY_BUFFER, _triangleVertexBuffer);      
       gl_ctx.bindBuffer(gl_ctx.ELEMENT_ARRAY_BUFFER, _triangleFacesBuffer);
       gl_ctx.drawElements(gl_ctx.TRIANGLES, vertixesCounts, gl_ctx.UNSIGNED_SHORT, 0);
       gl_ctx.flush();
...
$("form").submit(function(event){
    depth = parseInt(event.currentTarget[0].value);
    runWebGLSierp();
})